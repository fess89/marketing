class Marketing::Optin
	...
	def validate(params)
		res = true
		#TODO: validate email
		res = (!params[:email].empty?) && res
		res = (!params[:mobile].empty?) && res
		res = (!params[:first_name].empty?) && res
		res = (!params[:last_name].empty?) && res
		res = (!params[:company_name].empty?) && res
		res = @@PERMISSION_TYPES.include?(params[:permission_type]) && res
		res = @@CHANNEL_TYPES.include?(params[:channel]) && res
	end
